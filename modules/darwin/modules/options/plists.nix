...
in {
  ...
  config = mkIf (enabled && cfg != { }) {
    system.activationScripts.postActivation.text = let
      enquote = str:
        "'" + lib.strings.removeSuffix "'" (lib.strings.removePrefix "'" str)
        + "'";
      wrapPath = str:
        builtins.concatStringsSep ":"
        (map enquote (lib.strings.splitString ":" str));
      toValue = obj: if isBool obj then boolToString obj else obj;

      toCmd = file: path: value: ''
        $DRY_RUN_CMD /usr/libexec/PlistBuddy -c "Set ${wrapPath path} ${
          toValue value
        }" "$HOME/${file}"
      '';
      toCmds = file: settings:
        concatStrings (mapAttrsToList (toCmd file) settings);
    in ''
