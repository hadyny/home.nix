...
let
  ...
  ssoSessionProfileType = types.submodule {
    options = {
      ...
      region = mkOption {
        type = types.str;
        description = "AWS Region to use for this profile";
      };
      output = mkOption {
        type = types.str;
        default = "json";
  ...
  ssoSessionType = types.submodule {
    options = {
      sso_startUrl = mkOption {
        type = types.str;
        description = "SSO Start URL: https://xxxx.awsapps.com/start";
      };
      sso_region = mkOption {
        type = types.str;
        description = "AWS Region to use for SSO";
      };
      sso_registration_scopes = mkOption {
        type = types.str;
        default = "sso:account:access";
      };
      profiles = mkOption {
        type = types.attrsOf ssoSessionProfileType;
        default = { };
      };
    };
  };
  ...
  config = mkIf cfg.enable {
    ...
    home.file.".aws/config".text =
      let
        ...
        external = lib.mapAttrs' (name: value: {
          name = "profile ${name}";
          value = toCredProcess value;
        }) cfg.externalCredentials;
      in
