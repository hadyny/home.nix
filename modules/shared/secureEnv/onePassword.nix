  ...
  populateSecrets = namespace: variables: sshKeys:
    let
      ...
      syncOneVariable = key: item: ''
        ...
        ${storePasswordCmd namespace key "$password"
        "Password from ${item.vault}.${item.item}.${item.field}"}
      '';

      syncAllVariables = items:
    ...
    in pkgs.writeShellScript "populateKeys" ''
      set -e
...
in {
  options.secureEnv.onePassword = {
    ...
    namespace = mkOption {
      ...
      default = "nixConfig";
      description =
        "Namespace for keychain entries (displays as 'account' in Keychain)";
    };
  ...
  config =
    mkIf (cfg.enable && (cfg.sessionVariables != { } || cfg.sshKeys != { })) {
      home.activation.passwordsToKeychain =
        hm.dag.entryAfter [ "writeBoundary" ] ''
          noteEcho "Populating keychain from 1Password";
          $DRY_RUN_CMD ${
            populateSecrets cfg.namespace cfg.sessionVariables cfg.sshKeys
          }
        '';

      home.sessionVariables =
        lib.mapAttrs (k: v: "$(${lookupPasswordCmd cfg.namespace k})")
